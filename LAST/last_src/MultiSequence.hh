...
class MultiSequence{
 public:
  ...
  void reinitForAppending();
		...
		void fromMemory(const std::map<std::string, char *> &db_buffers,
		                indexT seqCount,
  ...
  void toFiles( const std::string& baseName, std::map<std::string, char*> &dbs,
  ...
  //std::istream& appendFromFasta( std::istream& stream, indexT maxSeqLen);
		...
		std::size_t appendFromFasta(char *&query_block);
  //bool appendFromFastaLASTDB( std::istream& stream, indexT maxSeqLen, bool unlimited );
		//std::size_t appendFromFastaLASTDB( char *&ptr, indexT maxSeqLen);
...
*/
  ...
  std::size_t readFastaName(std::istream &stream);
  std::size_t readFastaName(char *query_block);
